...
`;
  ...
  if (container && follower) {
    ...
    follower.style.transition =
      "transform 0.5s cubic-bezier(0.42, 0, 0.58, 1), left 0.1s, top 0.1s"; // Smooth grow-in easing
    ...
    container.addEventListener("mousemove", (e) => {
      ...
      targetX = Math.max(
        followerRadius,
        Math.min(mouseX - followerRadius, rect.width - followerRadius)
      );
      targetY = Math.max(
        followerRadius,
        Math.min(mouseY - followerRadius, rect.height - followerRadius)
      );
